...
同様に閾値を下げていく
'''
def find_peak_point(xdata, ydata, threshold, cut_range):
    ...
    for ii in peak_idx[0]:
        low_idx = ii - cut_range            # 左端のインデックスを取得
        high_idx = ii + cut_range           # 右端のインデックスを取得
...
# find spike timing
# =============================================================================
cut_range = 50          # カットする範囲を決める
peak_high_x, peak_high_y, ydata_high_off, peak_idx_high = find_peak_point(ttime, spike_waveform_filtered, threshold_high, cut_range)
peak_middle_x, peak_middle_y, ydata_middle_off, peak_idx_middle = find_peak_point(ttime, ydata_high_off, threshold_middle, cut_range)
peak_low_x, peak_low_y, ydata_low_off, peak_idx_low = find_peak_point(ttime, ydata_middle_off, threshold_low, cut_range)
...
ax2.plot(ttime, thr_highArr, color = 'b')
ax2.plot(ttime, thr_middleArr, color = 'r')
ax2.plot(ttime, thr_lowArr, color = 'g')
...
normalized_time = np.linspace(-0.5, 0.75, cut_range*2)
