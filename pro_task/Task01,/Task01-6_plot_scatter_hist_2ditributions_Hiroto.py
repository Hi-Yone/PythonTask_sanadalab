...
y1 = np.random.normal(0, 1, 5000)
x2 = np.random.normal(3, 1, 5000)
y2 = np.random.normal(3, 1, 5000)
...
x2_hist, x2_bins = np.histogram(x2, bins = 40, range = (-6,6))
y2_hist, y2_bins = np.histogram(y2, bins = 40, range = (-6,6))
newX2 = [(x2_bins[i-1]+x2_bins[i])/2 for i in range(1, len(x2_bins))]
newY2 = [(y2_bins[i-1]+y2_bins[i])/2 for i in range(1, len(y2_bins))]

ax1.bar(newX2, x2_hist, width = 0.25, color = 'b', alpha = 0.5, label = 'x2')
ax2.scatter(x2, y2, color = 'b', alpha = 0.5, label = 'x2VSy2')
ax3.barh(newY2, y2_hist, height = 0.25, color = 'b', alpha = 0.5, label = 'y2')
